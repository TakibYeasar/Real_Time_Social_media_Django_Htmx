...
from .forms import *
from core.forms import ReplyCreateForm
from inbox.forms import InboxNewMessageForm


def profile_view(request, username=None):
    if username:
        profile = get_object_or_404(AUTH_USER_MODEL, username=username).profile
    else:
        try:
    ...
    if request.htmx:
        if 'top-posts' in request.GET:
            posts = profile.user.posts.annotate(num_likes=Count('likes')).filter(
                num_likes__gt=0).order_by('-num_likes')
        elif 'top-comments' in request.GET:
            comments = profile.user.comments.annotate(num_likes=Count(
                'likes')).filter(num_likes__gt=0).order_by('-num_likes')
...
def profile_edit_view(request):
    ...
    if request.path == reverse('profile-onboarding'):
        template = 'auth/profile_onboarding.html'
    else:
...
def profile_delete_view(request):
    ...
    return render(request, 'auth/profile_delete.html')
