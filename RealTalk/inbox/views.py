...
def search_users(request):
    if request.htmx:
        ...
        if len(letters) > 0:
            ...
            users = AUTH_USER_MODEL.objects.filter(
                Q(username__icontains=letters) | Q(id__in=users_id)
            ).exclude(username=request.user.username)
            return render(request, 'core/list_searchuser.html', {'users': users})
        else:
            return HttpResponse('')
    else:
        raise Http404()
...
def new_message(request, recipient_id):
    recipient = get_object_or_404(AUTH_USER_MODEL, id=recipient_id)
    ...
    return render(request, 'core/form_newmessage.html', context)
...
def new_reply(request, conversation_id):
    ...
    return render(request, 'core/form_newreply.html', context)
...
def notify_newmessage(request, conversation_id):
    ...
    latest_message = conversation.messages.first()
    if conversation.is_seen == False and latest_message.sender != request.user:
        return render(request, 'core/notify_icon.html')
    else:
        return HttpResponse('')
